...
# CALM-PDE model
from models.models_airfoil import CALM_PDE_Airfoil
...
def run_training(config=None, wandb_config=None):
    ...
    model = CALM_PDE_Airfoil(neural_ode, in_channels=in_channels, out_channels=out_channels,
                             init_mesh=train_data.grid[0, :, 0, :], dropout=config.dropout).to(device)
    ...
    for ep in range(config.epochs):
        ...
        model.train()
        for x, node_type, y, pos in train_loader:
            # b, n, t, c
            x = x.to(device)
        ...
        model.eval()
        with torch.no_grad():
            for x, node_type, y, pos in test_loader:
                # b, n, t, c
                x = x.to(device)
                node_type = node_type.to(device)
                y = y.to(device)
